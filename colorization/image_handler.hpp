...
public:
    ...
    int remap_histogram(LabImage &target, const CHANNEL &channel);
    int remap_histogram(LabImage &target, Swatch &swatch, const CHANNEL &channel);
...
int LabImage::remap_histogram(LabImage &target, Swatch &swatch, const CHANNEL &channel = L)
{
    ...
    {
        for (int w = swatch.target_minw; w < swatch.target_maxw; w++)
        {
            if (this->get_pixel(w, h, channel, pixel_val))
            {
                printf("Fail to remap histogram.\n");
...
int LabImage::jitter_sampling(const int &sample_num, std::vector<std::pair<int, int>> &res_arr, int min_w, int max_w, int min_h, int max_h) const
{
    ...
    int h_step = (int)((max_h - min_h) / n), w_step = (int)((max_w - min_w) / n);
    ...
    {
        for (int w = min_w; w < max_w; w += w_step)
        {
            value = u_rand(random);
    ...
    for (int w = min_w; w < max_w; w++)
    {
        for (int h = min_h; h < max_h; h++)
        {
            ...
            for (auto iter = pixel_arr.begin(); iter != pixel_arr.end(); iter++)
            {
                ...
                double tmp_e = (double)abs(pixel_val - src_pixel_val) * luminance_ratio + (double)abs(sig_val - src_sig_val) * (1.0 - luminance_ratio);
                if (tmp_e < e)
                {
                    e = tmp_e;
                    best_w = src_w;
